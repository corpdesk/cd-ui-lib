...
import { format, parseISO, addDays, addSeconds } from 'date-fns';
...
import { SocketIoService, WebsocketService, WsHttpService } from '../cd-push';
...
export class SessService {
  ...
  */
  ...
  resetExprTime(ttl: any) {
    ...
    const exprTime = addSeconds(new Date(), ttl);
    localStorage.setItem('ExprTime-' + this.token, exprTime.toString());
  }

  getExprTime(ttl: any) {
    return addSeconds(new Date(), ttl).toString();
  }
