...
export class AclService {
    ...
    async initComponent(params: any, iClient: any, svSess: any) {
        ...
        this.log(`params:${JSON.stringify(params)}`, 'initComponent')
        let ret = false;
        if (asStr) {
            if(iClient.baseModel){
                ...
                this.log('16','initComponent')
                const message = 'baseModel was not initialized'
                console.error(`AclService::initComponent:error:`, message)
                const params = {
